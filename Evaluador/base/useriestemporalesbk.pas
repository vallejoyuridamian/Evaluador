...
type
  ...
  public
    ...
    seriesY: TDAOfVectR; // Series de salida
    seriesX: TDAOfVectR; // Series de entrada
...
{$ENDIF}
    ...
    constructor CreateVacia(dtPrimeraMuestra: TDateTIme;
      PeriodoDeMuestreo_Horas: NReal; nPuntos: integer);
    ...
    // Obtiene un puntero a la serie kSerie.
...
{$ENDIF}
    ...
    begin
      ...
      fk:= NextFloat( r );
      for kserie := 1 to nSeries do
        series[kserie - 1].pv[kpunto]:= nextFloat( r );
    end;
...
begin
  ...
  for k := 0 to high(self.tipo_Serie) do
    if tipo_Serie[k] = ENTRADA then
      Inc(cnt_Entradas)
    else
      Inc(cnt_Salidas);

  setlength(seriesX, cnt_Entradas);
  setlength(seriesy, cnt_Salidas);
  ...
  for k := 0 to high(self.tipo_Serie) do
    if tipo_Serie[k] = ENTRADA then
    begin
      seriesX[cnt_Entradas] := series[k];
      NombresSeriesX[cnt_Entradas] := nombresSeries[k];
      Inc(cnt_Entradas);
    end
    else
    begin
      seriesY[cnt_Salidas] := series[k];
      NombresSeriesY[cnt_Salidas] := nombresSeries[k];
      Inc(cnt_Salidas);
    end;
...
begin
  ...
  for k := 0 to NSeriesX - 1 do
    res.series[k] := TVectR.Create_Clone(seriesX[k]);
  res.actualizarAuxiliares;
  Result := res;
end;
...
begin
  for kPunto := 1 to NPuntos do
    if (series[kSerie_Dir].e(kPunto) < 0) or (series[kSerie_Vel].e(kPunto) < 0) then
    begin
      valor_filtro := min(series[kSerie_Dir].e(kPunto), series[kSerie_Vel].e(kPunto));
      series[kSerie_Dir].pon_e(kPunto, valor_filtro);
      series[kSerie_Vel].pon_e(kPunto, valor_filtro);
    end
    else
    begin
      Vel_x := -sin(series[kSerie_Dir].e(kPunto) * pi / 180) *
        series[kSerie_Vel].e(kPunto);
      Vel_y := -cos(series[kSerie_Dir].e(kPunto) * pi / 180) *
        series[kSerie_Vel].e(kPunto);
      series[kSerie_Dir].pon_e(kPunto, Vel_y);
      series[kSerie_Vel].pon_e(kPunto, Vel_x);
    end;
...
begin
  ...
  setlength(series, nseries);
  if vdatos = nil then
    series[nseries - 1] := TVectR.Create_init(self.NPuntos)
  else
    series[nseries - 1] := vdatos;
...
begin
  ...
  for k := 0 to saux.NSeries - 1 do
  begin
    nombre := saux.nombresSeries[k];
    val := saux.series[k];
    self.AddSerie(nombre, val);
    self.NPuntos := saux.NPuntos;
  end;
end;

function TSeriesDeDatos.GetSerie(kSerie: integer): TVectR;
var
  k, nSeriesAdicionales: integer;
begin
  nSeriesAdicionales:= kSerie- (self.NSeries-1);
  if nSeriesAdicionales > 0 then
  begin
    for k := 0 to nSeriesAdicionales - 1 do
      AddSerie( 'sa_'+IntToStr( NSeries ), TvectR.Create_Init( NPuntos));
  end;
  result:= series[ kSerie ];
end;
...
begin
  ...
  begin
    for k := 0 to nSeriesAdicionales - 1 do
      self.AddSerie(xserie.nombresSeries[
       k + (xserie.NSeries - nSeriesAdicionales) ], TvectR.Create_Init(xserie.NPuntos));
  end;


  for k := 0 to xserie.NSeries - 1 do
  begin
    xserie.series[k].limpiar_huecos(huecos, defval);
    self.series[k + offset].sum( xserie.series[k] );
  end;
end;
...
begin
  ...
  prom:= 0; vrz:= 0; // solo para evitar warning
