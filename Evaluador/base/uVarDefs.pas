...
{  TTypeOfVar= ( TVal_Error, TVal_S, TVal_NR, TVal_NI, TVal_VR,
  ...
  public
    ...
    constructor CreatePpdIndice(cosa: Pointer{^TCosa};
      const xnombreVar, unidades: string; pVariable: Pointer;
      pVector: Pointer; Ppd: Pointer); virtual;


    // si el nombre de la variable termina en [n] o en _Pn retorna
    // el nombre sin esas terminaciones. Si no encuentra ese tipo
    // de terminación retorna el string vació.
    function nombre_vect: string;

    property valStrPersistible: string read getValorAsStringPersistible
      write setValorFromStringPersistida;
  end;
  ...
  public
    ...
    constructor Create(
      xCatalogoReferencias: TCatalogo;
      cosa: Pointer{^TCosa}; const xnombre: string;
  ...
  public
    constructor Create(cosa: Pointer{^TCosa}; const xnombre, xunidades: string;
      variable: PTDAofNInt; usarNomenclaturaPorPoste: boolean); reintroduce;
  ...
  public
    constructor Create(cosa: Pointer{^TCosa}; const xnombre, xunidades: string;
      variable: PTDAOfBoolean; usarNomenclaturaPorPoste: boolean); reintroduce;
...
{      function asS: String; override;
    ...
    constructor Create(cosa: Pointer{^TCosa}; const xnombre, xunidades: string;
      precision, decimales: integer; Ppd: Pointer; variable: PTDAOfNReal;
      usarNomenclaturaPorPoste: boolean);
      reintroduce;
  end;
...
{      function asS: String; override;
    ...
    constructor Create(cosa: Pointer{^TCosa}; const xnombre, xunidades: string;
      Ppd: Pointer; variable: PTDAOfNInt; usarNomenclaturaPorPoste: boolean);
      reintroduce;
  end;
...
{      function asS: String; override;
    ...
    constructor Create(cosa: Pointer{^TCosa}; const xnombre, xunidades: string;
      Ppd: Pointer; variable: PTDAOfBoolean; usarNomenclaturaPorPoste: boolean);
      reintroduce;
  end;
  ...
  private
    function Get(Index: integer): TVarDef;
  public
    ...
    property Items[Index: integer]: TVarDef read Get; default;
    function getNombres: TStrings;
  end;
...
// el nombre sin esas terminaciones. Si no encuentra ese tipo
// de terminación retorna el string vació.
function TVarDef.nombre_vect: string;
var
  res, s, sn: string;
  i1, i2: integer;
  n, rescod: integer;
  ...
  begin
    ...
    while buscando and (i1 > 0) do
      if s[i1] = '_' then
        ...
        Dec(i1);
    if not buscando and (s[i1 + 1] = 'P') then
    begin
      n := i2 - i1 - 1;
      if n > 0 then
      begin
...
constructor TVarDef_Referencia.Create(xCatalogoReferencias: TCatalogo;
  cosa: Pointer; const xnombre: string; cosaConNombre: Pointer);
begin
  inherited Create(cosa, xnombre, unidades, cosaConNombre);
  CatalogoReferencias:= xCatalogoReferencias;
end;
...
begin
  ...
  if (clase <> '?') then
    (CatalogoReferencias as TCatalogoReferencias).registrar_referencia(cosa, clase, nombreRef, pVariable)
  else
...
begin
  ...
  cadena := cadena + FloatToStrF(
    (PTDAOfNReal(pVariable)^)[high(PTDAOfNReal(pVariable)^)], formatoReales,
    precision, decimales);
  Result := cadena;
end;
...
constructor TVarDef_VNI.Create(cosa: Pointer; const xnombre, xunidades: string;
  variable: PTDAOfNInt; usarNomenclaturaPorPoste: boolean);
begin
  inherited Create(cosa, xnombre, xunidades, variable);
  self.usarNomenclaturaPorPoste := usarNomenclaturaPorPoste;
end;
...
constructor TVarDef_VB.Create(cosa: Pointer; const xnombre, xunidades: string;
  variable: PTDAOfBoolean; usarNomenclaturaPorPoste: boolean);
begin
  inherited Create(cosa, xnombre, xunidades, variable);
  self.usarNomenclaturaPorPoste := usarNomenclaturaPorPoste;
end;
...
constructor TVarDef_TStringList.Create(cosa: Pointer{^TCosa};
  const xnombre, xunidades: string; variable: PTStringList;
  usarNomenclaturaPorPoste: boolean);
begin
  inherited Create(cosa, xnombre, xunidades, variable);
  self.usarNomenclaturaPorPoste := usarNomenclaturaPorPoste;
...
function TListaVarDefs.Get(Index: integer): TVarDef;
begin
...
function TListaVarDefs.getNombres: TStrings;
var
  res: TStrings;
  k: integer;
begin
