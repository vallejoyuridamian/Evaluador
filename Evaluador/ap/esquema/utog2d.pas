...
uses
  LCLIntf, LCLType, LMessages, Controls, Classes, Graphics,
...
type
  ...
  TCanalGrafico2D = class
    ...
    procedure Polygon(const vertices: TVarArrayOfComplex);
    // draw and fill a closed Polygon
    procedure Polyline(const vertices: TVarArrayOfComplex);
    // draw an open poly-line (no fill)


    // invalidar un rectángulo para lograr que se redibuje
    // le agrega px_Radio en todos los sentidos para incluir los
    // posibles destaques.
    procedure InvalidarRectangulo(h: hwnd; c1, c2: NComplex;
...
(*
  ...
  *)
    ...
    procedure MouseUp(Button: TMouseButton; Shift: TShiftState; cx: NComplex); virtual;
...
begin
  ...
  begin
    if m1.r > m2.r then
      swapreales(m1.r, m2.r);
    if m1.i > m2.i then
      swapreales(m1.i, m2.i);
  end;
end;
...
begin
  ...
  begin
    if rcx2.m1.r < rcx1.m1.r then
      rcx1.m1.r := rcx2.m1.r;
    if rcx2.m2.r > rcx1.m2.r then
      rcx1.m2.r := rcx2.m2.r;
  end;
  ...
  begin
    if rcx2.m1.i < rcx1.m1.i then
      rcx1.m1.i := rcx2.m1.i;
    if rcx2.m2.i > rcx1.m2.i then
      rcx1.m2.i := rcx2.m2.i;
  end;
end;
...
procedure TCanalGrafico2D.Polygon(const vertices: TVarArrayOfComplex);
// draw and fill a closed Polygon
var
  Ps: array of TPoint;
  k: integer;
...
procedure TCanalGrafico2D.Polyline(const vertices: TVarArrayOfComplex);
// draw an open poly-line (no fill)
var
  Ps: array of TPoint;
  k: integer;
...
procedure TCanalGrafico2D.InvalidarRectangulo(h: hwnd; c1, c2: NComplex;
  scroll_x, scroll_y: integer);
var
  r: TRect;
begin
...
var
  xband, yband: boolean;
begin
  xband := enorden(marco.m1.r, rcx.m1.r, marco.m2.r) or
    enorden(marco.m1.r, rcx.m2.r, marco.m2.r);
  xband := enorden(marco.m1.i, rcx.m1.i, marco.m2.i) or
    enorden(marco.m1.i, rcx.m2.i, marco.m2.i);
end;
...
procedure TOG2D.MouseDown(Button: TMouseButton; Shift: TShiftState; cx: NComplex);
begin
end;
...
procedure TOG2D.MouseUp(Button: TMouseButton; Shift: TShiftState; cx: NComplex);
begin
end;
...
procedure TOG2D_Grupo.MouseDown(Button: TMouseButton; Shift: TShiftState;
  cx: NComplex);
var
  k: integer;
begin
...
procedure TOG2D_Grupo.MouseUp(Button: TMouseButton; Shift: TShiftState; cx: NComplex);
var
  k: integer;
begin
  for k := 0 to high(integrantes) do
